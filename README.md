...
准确的说是将所有显示的功能提取为变量以实现一键更换功能

1. 在`style.css`中创建变量和值，以颜色为例子，设置一个变量和对应的值，并添加对应的替换规则类
...
2. 在`tailwind.config.js`创建自定义的类
   ```js
   // tailwind.config.js
   ...
     theme: {
       extend: { // 表示其它使用默认
           textColor:{ // 要添加的属性，对应使用类名前缀，如textColor对应tailwind的类名前缀text
               skin: { // 自定义名称，增加层级，此层不是必须
                   base: `var(--color-text-base)`, // base为自定义名称，值需要引用，最终类名为text-skin-base
               }
           }
       },
   },
...
3. 在需要处使用
   ```html
   <!-- 添加移除或更改theme-dark类即可更改实际显示的值 -->
   <a class="text-skin-base theme-dark"></a>
   <!-- 或者将样式类包裹在最外面 -->
   <div class="theme-dark"><a class="text-skin-base"></a></div>
   ```

### 自定义窗口

自定义窗口即不使用系统的窗口样式, 使用自定义的 UI 并实现窗口功能
...
Vue3 组合式函数注册和调用全局属性, 在`vue`包的`ComponentCustomProperties`下有文档实例
...
- 创建前端 ui 文件夹和后端 rust 文件夹平级的项目结构, 项目使用`vite`+`vue`+`typescript`
