...
class LogReceiveActivity : BaseBigTextTitleActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bind.receiveLogBtn.setOnClickListener {
            when (state) {
                ...
                State.RUNNING_BACKGROUND, State.SHOWING -> {
                    stop()
                    updateState()
                }
            }
    ...
    private fun start() {
        ...
        saveActions(a)
        val actions = a.filter { it != null && it.isCheck }.map { it!!.action }.toTypedArray()
        LogReceiveHelper.INSTANCE.start(*actions)
        LogReceiveViewHelper.INSTANCE.start()
    }

    private fun saveActions(actions: MutableList<LogActionVB?>) {
        DataHelper.LogReceive.saveActions(actions.filterNotNull())
    }
